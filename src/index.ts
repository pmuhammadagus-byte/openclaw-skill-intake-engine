...
function analyzeText(text: string) {
  ...
  let risk = "LOW";
  if (findings.includes("credential/config access") || findings.includes("shell execution")) risk = "MEDIUM";
  if (findings.includes("destructive operations")) risk = "HIGH";
    ...
    tool({
      ...
      async execute({ path: inputPath }, config) {
        ...
        const recommendation =
          result.risk === "HIGH" ? "QUARANTINE_AND_REVIEW" :
